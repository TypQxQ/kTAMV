...
class kTAMV_DetectionManager:
    ...
    ##### Setup functions
    # init function
    def __init__(self, config, io, *args, **kwargs):
        # send calling to log
    ...
    def burstNozzleDetection(self):
        ...
        # Wait for the toolhead to stop moving and then pause for 1 second to stabilize the image
        # Moved this before the while loop because we poll frames at request, not at a fixed rate
        ...
        while(detectionCount < 3):
            frame, size = self.io.get_single_frame()

            self.__uv, self.frame = self.nozzleDetection(frame)
        ...
        if(average_location[0] is not None):
            ...
            logging.debug("Nozzle recursivley detected at: %s" % str(self.__uv))
        else:
            self.__uv = None
